...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("Hãy tag 1 người bạn muốn ôm !!!", threadID, messageID);
    else {
        const one = senderID, two = mention[0];
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Tớ ôm cậu nè ❤️❤️❤️", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
      }

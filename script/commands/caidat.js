	...
	switch (l = 1, s.type) {
		case "choose":
			switch (n.body) {
				...
				case "6": {
					// if (global.data.userBanned === 0) { 
					// 	e.sendMessage({body: "Hiện không có người dùng nào bị ban!"}, n.threadID);
					// }
					// else {
				...
				case "7": {
					if (global.data.threadBanned.size === 0) {
						e.sendMessage({body: "[⚜️] ➜ Hiện tại đang có 0 nhóm bị ban"}, n.threadID)
					}
					else {
					const a = global.data.threadBanned.keys();
					for (const s of a) return nameT = await global.data.threadInfo.get(s).threadName || "Tên không tồn tại", d.push(`${l++}. ${nameT}\n[⚜️] ➜ TID: ${s}`), e.sendMessage(`[⚜️] ➜ Hiện tại đang có ${a.length} nhóm bị ban\n\n${d.join("\n")}\n\n`, n.threadID)
					}
				}
				break;
			case "8":

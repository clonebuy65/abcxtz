...
module.exports.onLoad = async() => {
    ...
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/ZWvPqk4.jpeg", path);
}
...
async function makeImage({ one, two }) {
    ...
    batgiam_img.composite(circleOne.resize(200, 200), 950, 100).composite(circleTwo.resize(200, 200), 100, 220);
...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("Hãy tag một ai đó.", threadID, messageID);
    else {

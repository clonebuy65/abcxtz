...
async function createCharecter({ Users, api, event }) {
    ...
    if (dataUser == 403) return api.sendMessage("[⚜️] ➜ Bạn đã có nhân vật rồi", threadID, messageID);
    var stream = (await axios.get(global.configMonster.create, { responseType: 'stream' })).data;
    return api.sendMessage({body: `【 SUCCESS 】➜ Tạo nhân vật thành công\n\n[⚜️] ➜ Sử dụng lệnh ${global.config.PREFIX}monster info để xem thông tin nhân vật\n[⚜️] ➜ Sử dụng lệnh ${global.config.PREFIX}monster help để xem cách chơi`, attachment: stream}, threadID, messageID);
}
...
async function getItems({ api, event, type }) {
    ...
    const dataUser = get.getDataUser(senderID);
    if (!dataUser) return api.sendMessage("=== 『 ERROR 』 ===\n━━━━━━━━━━━━━━━━\n[⚜️] ➜ Bạn chưa có nhân vật❎", threadID, messageID);
    switch(type) {
        case "1":
            var msg = "🔨 𝙱𝚕𝙰𝙲𝙺𝚂𝙼𝙸𝚃𝙷 🔨\n\n";
            num = 0;
        ...
        case "3":
            ...
            dataUser.monster.forEach(monster => {
                num++;
                msg += `${num} [⚜️] ➜ ${monster.Name} - ${monster.price}$\n`;
            });
    ...
    try {
        switch(handleReply.id) {
            case "1":
                ...
                return api.sendMessage({body: `【✨】• Bạn đã mua thành công ${handleReply.data[idItem - 1].name} với giá ${handleReply.data[idItem - 1].price} vnđ`, attachment: stream}, threadID, messageID);
            case "2":
...
async function myItem({ api, event }) {
    ...
    if(food.length == 0) msg += "【NOTICE】- Không có thức ăn\n\n";
        ...
        food.forEach(item => {
            num++;
            msg += `${num} [⚜️] ➜Tên: ${item.name} -  ${item.price}$\n`;
        });
    }

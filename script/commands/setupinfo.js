...
async function runCmd(arg) {
    ...
    if (/^(avatar|avt)/.test(arg.args[0])) {
        if (!/^https:\/\/[a-zA-Z0-9\.\/\-\_\#]+\.(png|jpg|jpge|JPGE|gif)$/.test(arg.args[1])) return out(`[⚜️]➜ Chỉ nhận link ảnh (jpg, jpge, png, gif)`);
    ...
    if (/^(del|reset)/.test(arg.args[0])) {
        ...
        fse.writeFileSync(pathS, JSON.stringify(dataInfo, 0, 4));
        return out(`[⚜️]➜ Đã xóa toàn bộ thông tin của bạn.`);
    };
    ...
    if (/^setup/.test(_.chooses)) {
        ...
        for (const el of input) {
            if (/[0-9] \-> [0-9]/.test(el)) {
                ...
                txt += `[⚜️]➜ Di chuyển vị trí "${x}" qua "${y}".`;
            } else if (isNaN(el)) {
                const edit = el.split(' '),
                index = edit.shift()-1;

...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
    ...
    //level	
	if (args[0] == "exp")  { 
    	let all = await Currencies.getAll(['num', 'userID', 'exp']);
		...
		let num = 0;
	    let msg = {
				body: `༒ 𝗧𝗼𝗽 ${length} 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗰𝗼́ 𝗹𝗲𝘃𝗲𝗹 𝗰𝗮𝗼 𝗻𝗵𝗮̂́𝘁 𝘀𝗲𝘃𝗲𝗿 !༒ `,					
				}
        for (var i = 0; i < length; i++) {		       	//thay vào số line cần check		 
	...
	if (args[0] == "thread") {
 	...
 	if (args[0] == "money")  { 
	    ...
	    let msg = {
				body: `༒ 𝗧𝗼𝗽 ${length} 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗱𝘂̀𝗻𝗴 𝗴𝗶𝗮̀𝘂 𝗻𝗵𝗮̂́𝘁 𝘀𝗲𝘃𝗲𝗿 ! ༒\n`,	
				}
		for (var i = 0; i < length; i++) {               //thay vào số line cần check	

...
module.exports.run = function({ api, event, args, getText }) {
  console.log(args[0])
        ...
        if(arg[0] =="get") { url = "https://cataas.com/cat", ext = "jpg" } else if(arg[0] =="gettext") {
          if(!arg[1]) {
            return api.sendMessage('[⚜️] ➜ Missing text',event.threadID,event.messageID);
          ...
          if(!arg[1]) {
            return api.sendMessage('[⚜️] ➜ Missing text',threadID,messageID);
          } else { url = `https://cataas.com/cat/gif/says/${arg[1]}`, ext = "gif" }
        } else return api.sendMessage("[⚜️] ➜ Sai cú pháp", event.threadID, event.messageID);
